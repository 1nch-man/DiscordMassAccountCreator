...
def randString(length):
    re = ''.join(random.choice(string.ascii_letters + string.digits + emojis) for _ in range(length))
    return re
...
def create_account(key='', email=None, username='', cookie=None, fingerprint=None, verbose=True):
    ...
    jsondata = {
        ...
        "email":random_string(random.randint(3, 16))+'@gmail.com',
        "password":trash_string(8), # Need to be 6 or higher
        "invite":"yTpUtQMx",
        "consent":True,
    ...
    if verbose == True:
        if r.status_code == 201:
            ...
            wat = requests.get(url='https://discord.com/channels/@me',headers={'authorization': json.loads(r.content)['token']})
            if wat.status_code == 201:
              print(f'{Fore.GREEN} Account is verified{Fore.WHITE}')
            else:
              print(f'{Fore.RED} Account is not verified{Fore.WHITE}')
            sleep(120)
            create_account()
        else:
            lock.acquire()
            ...
            print(Fore.YELLOW + "[{}] Response:".format(r.status_code), json.loads(r.content['message']), Fore.WHITE)
            sleep(json.loads(r.content)['retry_after']-(-1))
            create_account()
    tries += 1
